...
All fees collected by Warp are burnt. They are estimated based on 3 factors: A creation fee based on the queue size, a maintenance fee based on the job duration, and a burn fee based on the reward amount. 
...
The current contract config is as follows:
