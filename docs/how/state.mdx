...
6. Jobs only remain active in the queue based on their duration, which is specified during job creation. Job duration is a variable in [fee estimation](./fees.html). When a job's duration runs out, it is [_`Evicted`_](focus://14[22:31],15[6:31],16[22:31]). 
