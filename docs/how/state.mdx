...
6. [Evictors](./overview.mdx#eviction) will collect 5% of the reward amount from the creator's Warp account balance for a job in the queue every 24 hours. If the account cannot pay, the job becomes [_`Evicted`_](focus://14[22:31],15[6:31],16[22:31]), and the 5% eviction fee is taken from the reward and sent to the evictor. The rest of the reward is refunded to the creator. 
