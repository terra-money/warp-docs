...
In order to keep a job active, _`0.01`_ Luna will be withdrawn from the job creator's Warp account every 24 hours. Keepers called "Evictors" will check each job in the queue daily, and collect the  _`0.01`_ Luna as a reward. When the Job creator's account no longer holds enough Luna, the job will be evicted and removed from the queue. The Evictor will receive  _`0.01`_ Luna from the Job's reward, and the Creator will be refunded the Job reward amount, minus the eviction fee. 
