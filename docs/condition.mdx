...
Conditions represent event definitions, and they outline the circumstances under which a job can be completed. A condition is a recursive data type that allows for multiple conditions within it, which all need to resolve to true for the Job to allow execution. This complex data type includes several underlying types.

A condition is either a combination of conditions or a single expression. Conditions can be nested and chained using regular boolean operators, such as _`And`_, _`Or`_, or _`Not`_. 
