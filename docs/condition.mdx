...
A value is the most primitive type of input used in an expression. In double-sided [general expressions](#general-expressions), the value can either be a primitive type or a query that gets a primitive type. 
