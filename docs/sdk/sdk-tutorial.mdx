...
The [Warp SDK](https://github.com/terra-money/warp-sdk) provides a simple way to interact with the Warp protocol's contracts. Developers can use the Warp SDK to easily create jobs, conditions, and variables in a typesafe environment. 

The Warp SDK can be thought of as a new programming language, introducing condition, variable, and job syntax. You can use the autocomplete functions in your IDE to help you get the most out of the Warp SDK. 

## Installation

To install the Warp SDK, run the following in your project's root. You may also need to install feather.js, Terra's JavaScript SDK, or the SDK of the chain you are using. 
...
1. Because this job is recurring, it requires a [variable](../how/variables.mdx) that needs to be updated upon every harvest and increment the harvest time by 1 day. Just like in other programming languages, you can provide variables in the Warp SDK that can be [updated](../how/variables.mdx#update-functions-and-loops). 
...
6. Next, the variable is given an initial `value`. This will be set to the time and date of the first harvest. The timestamp composer is called using `ts.date`, which takes in a JavaScript `Date` object as a parameter and returns a timestamp.
...
7. This job is recurring and contains a variable that needs to be [updated](../how/variables.mdx#update-functions-and-loops). The `.onSuccess()` method dictates that upon completion of a successful job, the `next_execution` timestamp value will be increased by 1 day by using an update function. 
...
8. Similarly, if the job should return an error upon execution, the timestamp value will be increased by an hour, and the job will be run, incrementing every hour upon failure until it is successful. 
...
9. The `.compose()` method is called at the end of a chain of method calls on a composer object to indicate that the composer object is complete.
...
3. The job is set to be recurring, which means it will be re-added to the queue after execution and the `next_execution` variable will be updated. 
