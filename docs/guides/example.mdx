...
# Job from scratch example

This example will walk you through creating a basic Warp job and condition without using a premade template. For general information on how Warp works, visit the [Overview page](../how/overview.mdx). For a tutorial on how to use the Warp App, visit the [Get started page](ui-tutorial.mdx).

## The workflow

1. [Create a Warp account](ui-tutorial.mdx). [Accounts](../how/overview.mdx#warp-accounts) are smart contracts that only you and your Warp jobs have permission to access.

2. [Add Luna](ui-tutorial.mdx#create-an-account) to your account.

3. [Create a job](ui-tutorial.mdx#create-a-job) with the swap parameters such as price per unit, how much Luna you're willing to spend, etc. For this example, suppose that you want to spend 1 Luna to receive a minimum of 9500 Astro. You can see the [job message in the example below](#the-job-message).
...
This example will use Astroport's Testnet Router to execute the job's swap. The Router allows you to use a single contract for multiple assets instead of having to keep track of pool addresses. For more information on queries, visit the [Query page](../how/query.mdx).
...
We specify that the minimum uAstro received should be _`9500000000`_, or 9500 \$Astro tokens. _`terra167dsqkh2alurx997wmycw9ydkyu54gyswe3ygmrs4lwume3vmwks8ruqnv`_ is the \$Astro token address.
